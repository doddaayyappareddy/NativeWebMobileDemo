        ...
        <View>
            {
                ...
                :
                    <TouchableOpacity style={[styles.wrapper,boxStyles]} onPress={() => { if(!dropdown){ slidedown() }else{ slideup() } }}>
                        <Text style={[{fontFamily},inputStyles]}>{ (selectedval == "") ? (placeholder) ? placeholder : 'Select' : selectedval  }</Text>
                        {
                            (!arrowicon)
                            ?
                                <Image 
...
const styles = StyleSheet.create({
    wrapper:{ borderWidth:0,borderBottomWidth:1,borderColor:'gray',paddingHorizontal:20,paddingVertical:1,flexDirection:'row',justifyContent:'space-between' },
    dropdown:{ borderWidth:1,borderRadius:10,borderColor:'gray',marginTop:1,overflow:'hidden'},
